...
SELECT full_name, COUNT(animals.*) FROM owners LEFT JOIN animals ON owners.id = animals.owner_id GROUP BY full_name;
...
SELECT animals.*, visits.visit_date FROM animals
...
SELECT  vets.name, COUNT(animals.*) FROM animals
LEFT JOIN visits ON animals.id = visits.animal_id
LEFT JOIN vets ON visits.vet_id = vets.id
WHERE vets.id = (
...
AND visits.visit_date BETWEEN 'Apr-01-2020' AND 'Aug-30-2020'
ORDER BY visit_date DESC;

-- What animal has the most visits to vets?
SELECT animals.name, COUNT(animals.id) FROM animals
LEFT JOIN visits ON animals.id = visits.animal_id
GROUP BY animals.id
...
WHERE (SELECT species_id FROM animals WHERE animals.id = visits.animal_id) != species_id;

-- What specialty should Maisy Smith consider getting? Look for the species she gets the most.
SELECT species.name, COUNT(animals.*) FROM animals
LEFT JOIN visits ON animals.id = visits.animal_id
LEFT JOIN vets ON visits.vet_id = vets.id
